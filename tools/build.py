...
def build():
    ...
    run([AAPT2, "link", "-o", OUT / "unsigned.apk", "-I", AJAR,
         ...
         # 版本策略：versionName 只随特性版本推进；versionCode 单调不减即可。
         # v1.0.2（code 107）= 1.0 后全部修复 + 调试日志开关。因 v1.0.1 名下
         # 曾发布过 code 105 的旧包，换名区分避免同号不同内容
         "--version-code", "107", "--version-name", "1.0.2"])
